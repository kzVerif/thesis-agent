...
```json
{
  ...
  "pid": 8120,
  "success": false,
  "message": "ไม่สามารถปิดโปรเซสได้",
  "error": "ไม่สามารถปิดโปรเซส PID 8120 ได้ กรุณาตรวจสอบว่าโปรเซสยังทำงานอยู่และ agent มีสิทธิ์เพียงพอ"
}
```

ฝั่ง server ควรตรวจ `type`, `action`, `pid` และ `success` เพื่อจับคู่ผลลัพธ์กับคำสั่ง ข้อความใน `message` และ `error` เป็นภาษาไทย หาก `success` เป็น `false` รายละเอียดจะอยู่ใน field `error`

ชื่อ field และค่าที่ใช้ควบคุม protocol เช่น `type`, `action`, `success` และ `kill_result` คงเป็นภาษาอังกฤษเพื่อให้เป็น machine-readable contract ที่แน่นอน ส่วนข้อความที่นำไปแสดงแก่ผู้ใช้จะเป็นภาษาไทย
